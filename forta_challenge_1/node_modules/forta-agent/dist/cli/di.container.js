...
function configureContainer(commandName, cliArgs) {
    ...
    var bindings = {
        ...
        commandName: awilix_1.asValue(commandName),
        fortaKeystore: awilix_1.asValue(path_1.join(os_1.default.homedir(), ".forta")),
        getFortaConfig: awilix_1.asFunction(get_forta_config_1.default),
        fortaConfig: awilix_1.asFunction(function (getFortaConfig) { return getFortaConfig(); }).singleton(),
        configFilename: awilix_1.asValue("forta.config.json"),
        localConfigFilename: awilix_1.asFunction(function (configFilename) {
        ...
        push: awilix_1.asFunction(push_1.default),
        disable: awilix_1.asFunction(disable_1.default),
        enable: awilix_1.asFunction(enable_1.default),
        keyfile: awilix_1.asFunction(keyfile_1.default),
        ...
        packageJson: awilix_1.asFunction(function () {
            try {
                var packageJsonPath = path_1.join(process.cwd(), "package.json");
                return utils_1.getJsonFile(packageJsonPath);
            }
            catch (e) {
                throw new Error("unable to parse package.json: " + e.message);
            }
        }).singleton(),
        agentName: awilix_1.asFunction(function (packageJson) { return packageJson.name; }).singleton(),
        agentId: awilix_1.asFunction(function (agentName) { return utils_1.keccak256(agentName); }).singleton(),
        version: awilix_1.asFunction(function (packageJson) { return packageJson.version; }),
        documentation: awilix_1.asValue(path_1.join(process.cwd(), 'README.md')),
        repository: awilix_1.asFunction(function (packageJson) {
        ...
        listKeyfiles: awilix_1.asFunction(list_keyfiles_1.default),
        addToIpfs: awilix_1.asFunction(add_to_ipfs_1.default),
        appendToFile: awilix_1.asFunction(append_to_file_1.default),
        getNetworkId: awilix_1.asFunction(get_network_id_1.default),
        getBlockWithTransactions: awilix_1.asFunction(get_block_with_transactions_1.default),
        getTransactionReceipt: awilix_1.asFunction(get_transaction_receipt_1.default),
        ...
        jsonRpcUrl: awilix_1.asFunction(function (fortaConfig) {
        ...
        ethersProvider: awilix_1.asFunction(function (jsonRpcUrl) { return new ethers_1.ethers.providers.JsonRpcProvider(jsonRpcUrl); }).singleton(),
        ethersAgentRegistryProvider: awilix_1.asFunction(function (agentRegistryJsonRpcUrl) { return new ethers_1.ethers.providers.JsonRpcProvider(agentRegistryJsonRpcUrl); }).singleton(),
        ipfsGatewayUrl: awilix_1.asFunction(function (fortaConfig) {
            if (!fortaConfig.ipfsGatewayUrl) {
                throw new Error("no ipfsGatewayUrl provided in config");
            }
            return fortaConfig.ipfsGatewayUrl;
        }),
        ipfsGatewayAuth: awilix_1.asFunction(function (ipfsGatewayUrl, fortaConfig) {
